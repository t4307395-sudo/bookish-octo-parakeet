(() => {
  ...
  const links = items.map(([href, icon, label]) => `<a class="global-side-link ${current === href ? 'active' : ''}" href="${href}"><span>${icon}</span>${label}</a>`).join('');
  ...
  document.getElementById('globalMenuBtn').addEventListener('click', () => { drawer.classList.add('open'); overlay.classList.add('open'); });
  document.getElementById('globalClose').addEventListener('click', close); overlay.addEventListener('click', close);
